defmodule JauthnWeb.CredentialController do
  use JauthnWeb, :controller
    ...
    with {:ok, _} <-
      ...
      conn =
        if get_session(conn, :login) do
          ...
          [{_, login, _, _, _} | _] = Jauthn.User.get_by_user_id(maybe_user_handle)
  ...
  defp login_with_username(conn, username) do
    cred_ids = Jauthn.User.get_by_username(username)
  ...
  defp credentials_from_user_id(user_id) do
    for {_, _, cred_id, cose_key, _} <- Jauthn.User.get_by_user_id(user_id) do
      {cred_id, cose_key}
    end
  end
  ...
  defp cred_mapping_from_user_handle(user_id) do
    for {_, _, cred_id, _, maybe_aaguid} <- Jauthn.User.get_by_user_id(user_id), into: %{} do
      {cred_id, maybe_aaguid}
    end
  end
